<template>
  <div class="absolute inset-0 bg-gray-100 py-2 px-8">
    <div class="flex items-start w-full justify-between mt-6">
      <h2 class="inline-block text-2xl font-bold"><i class="fas fa-search"></i> Search Unsplash</h2>
      <div>
        <span class="text-right"><a
          ...
          ><i class="far fa-times-square"></i></a></span>
      </div>
    </div>

    <p>Tip: try for example <a
        href=""
        class="text-blue-700 underline"
        @click.prevent="searchFor('dark background')"
      >dark background</a> or <a
        href=""
        class="text-blue-700 underline"
    ...
    <div
      v-if="searchResults && searchResults.total > 0"
      class="w-full"
    >
      <h4 class="text-2xl font-bold mt-6">Results ({{ searchResults.total }})</h4>
      ...
      <div class="w-full">
        ...
        <input
          class="form-input inline-block w-32"
          v-model="currentPage"
        >
